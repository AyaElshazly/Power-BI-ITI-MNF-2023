...
select OrderDate,sum(TotalDue) as "Total4day"
from Sales.SalesOrderHeader
...
select avg (distinct ListPrice) as AvgPrice
from  HumanResources.Employee
...
select concat('the',Name,'is only! ',ISNULL(ListPrice,0))
from Production.Product
order by ListPrice
...
union all
select format(getdate(), 'dddd') --day as character
union all
select format(getdate(), 'MMMM') --month as character